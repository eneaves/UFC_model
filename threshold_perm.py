...
- Hace barrido de umbrales en Validación y selecciona:
...
Uso típico:
  python step5_threshold_perm.py --datadir out_rf \
    --n_estimators 200 --max_depth 12 --min_samples_leaf 10 --k_thresh 16 --m_try 9 \
    --oob True --metric f1 --n_repeats 5 --target_precision 0.70
"""
...
def sweep_thresholds(y_true, y_scores, start=0.0, stop=1.0, num=201):
    ...
    for t in thrs:
        ...
        precision, recall, f1, acc = prec_rec_f1_from_cm(cm)
        rows.append({"threshold": float(t), "precision": precision, "recall": recall, "f1": f1, "accuracy": acc,
...
def permutation_importance(rf, X_val, y_val, threshold, n_repeats=5, metric="accuracy", random_state=42):
    ...
    _, _, f1_base, acc_base = prec_rec_f1_from_cm(cm)
    ...
    for j in range(n_features):
        ...
        for _ in range(n_repeats):
            ...
            _, _, f1_p, acc_p = prec_rec_f1_from_cm(cm_p)
...
def main():
    ...
    ap.add_argument("--target_precision", type=float, default=None, help="Si se define, busca el umbral con precision>=target que maximiza recall")
    args = ap.parse_args()
    ...
    df_thr.to_csv(os.path.join(args.datadir, "threshold_sweep.csv"), index=False)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(args.datadir, "threshold_sweep.png"))
    plt.close()
    ...
    with open(os.path.join(args.datadir, "best_threshold.json"), "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)

    print("=== Threshold sweep ===")
    print(f"Best F1 at thr={best_row['threshold']:.3f} -> F1={best_row['f1']:.3f}, P={best_row['precision']:.3f}, R={best_row['recall']:.3f}, Acc={best_row['accuracy']:.3f}")
    if best_row_prec is not None:
        print(f"Best Recall with Precision>={args.target_precision:.2f} at thr={best_row_prec['threshold']:.3f} -> P={best_row_prec['precision']:.3f}, R={best_row_prec['recall']:.3f}, F1={best_row_prec['f1']:.3f}")
    ...
    importances, base_score = permutation_importance(rf, X_val, y_val, threshold=chosen_thr,
                                                     n_repeats=args.n_repeats, metric=args.metric, random_state=42)
    df_imp = pd.DataFrame({"feature": features, "perm_importance": importances}).sort_values("perm_importance", ascending=False)
    df_imp.to_csv(os.path.join(args.datadir, "perm_importance.csv"), index=False)

    # Plot top-20
    topk = min(20, len(features))
    order = np.argsort(importances)[::-1][:topk]
    plt.figure(figsize=(10,6))
    plt.barh(range(topk), importances[order])
    plt.yticks(range(topk), [features[i] for i in order])
    plt.gca().invert_yaxis()
    plt.xlabel(f"Drop in {args.metric} after permutation")
    plt.title(f"Permutation Importance (top {topk}) at threshold={chosen_thr:.3f}\nBase {args.metric}={base_score:.3f}")
    plt.tight_layout()
    plt.savefig(os.path.join(args.datadir, "perm_importance.png"))
    plt.close()

    print(f"Saved: threshold_sweep.csv/png, best_threshold.json, perm_importance.csv/png in {args.datadir}")
