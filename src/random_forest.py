...
Uso:
  ...
  python -m src.random_forest --datadir data --figdir outputs/figures --repodir outputs/reports \
    --n_estimators 200 --max_depth 12 --min_samples_leaf 10 --k_thresh 16 --m_try 9 --oob True
"""
...
from .utils import (
    ...
    plot_feature_importances_barh, plot_acc_vs_trees
)
...
class RandomForestScratch:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        self.feature_importances_ = fi_accum / fi_accum.sum() if fi_accum.sum() > 0 else np.zeros(p, dtype=float)
...
def run_sweep_estimators(n_list, X_train, y_train, X_val, y_val,
    ...
    val_accs, train_accs, oobs = [], [], []
    for n in n_list:
        rf_tmp = RandomForestScratch(
            n_estimators=n, max_depth=max_depth, min_samples_leaf=min_samples_leaf,
            m_try=m_try, k_thresh=k_thresh, bootstrap=bootstrap, oob=oob, random_state=random_state
        )
        t0 = time.time()
...
def main():
    ...
    ap.add_argument("--figdir",  type=str, default="outputs/figures", help="Carpeta para figuras")
    ap.add_argument("--repodir", type=str, default="outputs/reports", help="Carpeta para reportes JSON/CSV")
    ...
    # Matriz y métricas en Val
    cm_val = confusion_matrix(y_val, yhat_va)
    ...
    save_json(
        {"features": features,
         "importances": (rf.feature_importances_.tolist() if rf.feature_importances_ is not None else [])},
        Path(args.repodir) / "rf_importances.json"
    )

    # Curvas ROC/PR en Validation usando "probabilidades" (promedio de votos)
    y_scores_val = rf.predict_proba(X_val)
    roc_auc_val = plot_roc(y_val, y_scores_val, Path(args.figdir) / "roc_curve.png", label="RF scratch")
    ap_val = plot_pr(y_val, y_scores_val, Path(args.figdir) / "pr_curve.png", label="RF scratch")
    save_json({"roc_auc_val": roc_auc_val, "average_precision_val": ap_val}, Path(args.repodir) / "val_curve_scores.json")

    # Top importances barh
    plot_feature_importances_barh(features, rf.feature_importances_, Path(args.figdir) / "feature_importances.png", topk=20)
    ...
    save_json({
        ...
        "accuracy": {"train": float(accuracy(y_train, yhat_tr)),
                     "val":   float(accuracy(y_val,   yhat_va)),
                     "test":  float(accuracy(y_test,  yhat_te))},
        "confusion_matrix_val": cm_val.tolist(),
        "metrics_val": metrics_val
    }, Path(args.repodir) / "rf_summary.json")

    # Sweep de #árboles (figura)
    n_list = [10, 20, 40, 80, 120, 160, 200]
    train_accs, val_accs, oobs = run_sweep_estimators(
        n_list, X_train, y_train, X_val, y_val,
        max_depth=args.max_depth, min_samples_leaf=args.min_samples_leaf,
        m_try=(args.m_try or int(math.sqrt(len(features)))), k_thresh=args.k_thresh,
        bootstrap=args.bootstrap, oob=args.oob, random_state=RANDOM_SEED
    )
    plot_acc_vs_trees(n_list, train_accs, val_accs, oobs, Path(args.figdir) / "acc_vs_trees.png")

    print("Artefactos guardados en outputs/figures y outputs/reports.")
