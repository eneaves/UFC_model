...
- Soporta solo features numéricas (el preprocesamiento ya dejó todo en numérico).
- Usa selección aleatoria de features por nodo (m_try) y umbrales candidatos por percentiles.

Uso (ejemplos):
  #(por defecto lee data/ y guarda reporte en outputs/reports/)
  python -m src.decision_tree

  # o explícito:
  python -m src.decision_tree --datadir data --out_reports outputs/reports --max_depth 12 --min_samples_leaf 5 --k_thresh 16
"""
...
import numpy as np
import pandas as pd
from .random_forest import confusion_matrix
...
class DecisionTreeScratch:
    def __init__(self, max_depth=10, min_samples_leaf=5, m_try=None, k_thresh=16, random_state=42):
        ...
        self.m_try = m_try  # si None, usa sqrt(n_features)
...
def load_split(datadir: str):
    # ✅ NUEVAS RUTAS: por defecto leerá de "data/"
...
def maybe_save_report(path: str, payload: dict):
    try:
        os.makedirs(path, exist_ok=True)
        with open(os.path.join(path, "dt_val_report.json"), "w") as f:
            json.dump(payload, f, indent=2)
    except Exception as e:
        print(f"[WARN] No se pudo guardar el reporte en {path}: {e}")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--datadir", type=str, default="data", help="Carpeta donde están ufc_rf_{train,val,test}.csv")
    parser.add_argument("--out_reports", type=str, default="outputs/reports", help="(opcional) carpeta para guardar un JSON de métricas")
    ...
    acc_tr = accuracy(y_train, yhat_tr)
    acc_va = accuracy(y_val,   yhat_va)
    acc_te = accuracy(y_test,  yhat_te)

    cm_val = confusion_matrix(y_val, yhat_va)
    cm_test = confusion_matrix(y_test, yhat_te)
    ...
    print(f"Accuracy  Val  : {acc_va:.3f}")
    ...
    # ✅ Guardar reporte (opcional) en outputs/reports/
    maybe_save_report(args.out_reports, {
        ...
        "accuracy": {"train": acc_tr, "val": acc_va, "test": acc_te}
    })
